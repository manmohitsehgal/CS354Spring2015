...
#define	LF_NUM_DIR_ENT	200		/* num. of files in a directory	*/
#define FILE_DEPTH 200
...
#define LF_TYPE_FILE	0		/* ldentry type file		*/
#define LF_TYPE_DIR	1		/* ldentry type directory	*/
...
struct	ldentry	{			/* description of entry for one	*/
	...
	int32	type;			/* file type: dir or file	*/
	int32	isUsed;			/* Currently allocated?		*/
...
struct	lfdir	{			/* entire directory on disk	*/
	...
	ibid32	lfd_ilist;		/* Stores file name, i-block mapping	*/
	uint32	lfd_size;		/* Size of the directory	*/
	char	padding[496];		/* unused chars in directory blk*/
};
#pragma pack()
...
struct	lflcblk	{			/* Local file control block	*/
	...
	ibid32	firstId;		/* ID of the first index block	*/
	char	path[LF_NUM_DIR_ENT][LF_NAME_LEN];	/*path of the file	*/
	uint32	depth;			/* depth relative to the root	*/
};

extern	struct	lfdata	Lf_data;
extern	struct	lflcblk	lfltab[];

extern int head(int, char *);
...
#define LF_CTL_MKDIR	F_CTL_MKDIR
